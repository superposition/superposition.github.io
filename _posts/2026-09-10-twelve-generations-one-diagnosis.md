...
The generated templates carry the pipelined form now, so the loop can be asked to tune inside that structure rather than only around it. Rendered at the hand-written parameters they reproduce the kernel to within 1.1% — 67.69 against 68.44 µs — and eight generations starting from there were all refused: the width of the `A` copies and the staging layout move the pipelined kernel by less than a third of a percent, where the same two knobs were worth 1.6x before the copies overlapped the arithmetic. `k_step` 32 is confirmed, larger tiles are worse. The space around the pipeline is flat, which is where this thread closes: the next gain will not come from re-arranging it.

The loop's own view — event spans around the call — and the twelve-generation ledger with its rejections are in the [measurement record](https://github.com/superposition/mage/blob/master/docs/experiments/mage-005.md), along with the search space, the static constraints, the audit that renders and checks all 46 reachable configurations, and the regression tests. The captures are one file per paired session under `docs/assets/results/evolution-loop/kernel-time/`.
